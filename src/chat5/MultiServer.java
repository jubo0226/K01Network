...
public class MultiServer {
	...
	public void init() {
		try {
			...
			while(true) {
				...
				//쓰레드로 정의된 내부클래스 객채생성 및 시작
				Thread mst = new MultiSeverT(socket);
				mst.start();
			}
		}
		catch (Exception e) {
	...
	멤버메소드를 변경한다. 따라서 객체를 생성후 호출하는 방식으로 변경된다.
	 */
	...
	내부클래스
		...
		부분이 해당 내부클래스로 이동되었다.
	 */
	class MultiSeverT extends Thread {
		 ...
		 */
		public MultiSeverT(Socket socket) {
			this.socket = socket;
			try {
